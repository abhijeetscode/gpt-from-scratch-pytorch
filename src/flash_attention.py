...
class FlashAttention(nn.Module):
    def __init__(self, head_dim:int) -> None:
        ...
        assert self.context_length % self.tile_size == 0, "context length should be divisible by tile size"
    ...
    def forward(self, x: torch.Tensor, mask: torch.Tensor) -> torch.Tensor:
        ...
        output_buffer = torch.zeros((self.context_length, self.head_dim), device=x.device)
...
class MultiHeadAttention(nn.Module):
    def __init__(self) -> None:
        super().__init__()
        assert settings.embedding_dim % settings.num_heads == 0, "embedding dim should be divisible by num heads"

        assert settings.context_length % settings.tile_size == 0, "context length should be divisible by tile size"

        self.head_dim: int = settings.embedding_dim // settings.num_heads
        self.tile_size = settings.tile_size

        self.heads = nn.ModuleList([FlashAttention(self.head_dim) for _ in range(settings.num_heads)])
    ...
    def forward(self, x: torch.Tensor):
        ...
        assert x.shape[-1] == settings.embedding_dim, "feature dimension of output of MultiHeadAttention should match with embedding dim"
        return x
    

if __name__ == "__main__":
    # considering 2 dimensions only context_length, embedding_dim
    x = torch.randn((settings.context_length, settings.embedding_dim), device=settings.device)
